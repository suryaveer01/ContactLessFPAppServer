...
def enhance_fingerprint(img):
    ...
    clahe_img = cv2.adaptiveThreshold(clahe_img,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,\
                cv2.THRESH_BINARY,11,2)
    clahe_img = (255-clahe_img)
    clahe_img = cv2.medianBlur(clahe_img, 5)
    clahe_img = cv2.medianBlur(clahe_img, 3)
    clahe_img = cv2.medianBlur(clahe_img, 3)

    image_enhancer = FingerprintImageEnhancer() 
    enh_img, ridge_img = image_enhancer.enhance(img) 

    enh_img = 255*enh_img
    ridge = 255 * ridge_img.astype(np.uint8)

    

    enh_img = coherence_filter(ridge, sigma = 11, str_sigma = 53, blend = 1, iter_n = 5, blockSize=3)
    
    return clahe_img, ridge, enh_img

def main(image):
    clahe_img, ridge_img, enh_img = enhance_fingerprint(image)
    return image, clahe_img, ridge_img, enh_img